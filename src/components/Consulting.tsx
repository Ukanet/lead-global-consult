...
export default function Consulting() {
  return (
    <section id="consulting" className="py-24 md:py-32 bg-[#EEF2F6]" aria-labelledby="consulting-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">

          <AnimatedSection animation="slide-left">
            <div>
              <p className="text-[#C5A059] text-xs tracking-[0.3em] uppercase mb-4" style={{ fontFamily: "var(--font-mono)" }} aria-hidden="true">Educational Consulting</p>
              <h2 id="consulting-heading" className="font-display text-[#0D2346] text-3xl sm:text-4xl md:text-5xl font-bold mb-6" style={{ fontFamily: "var(--font-display)" }}>
                Helping Institutions Unlock Their Full Potential
              </h2>
              <p className="text-[#2B303A] text-lg leading-relaxed mb-6">
                LEAD GLOBAL CONSULTS specializes in comprehensive educational consulting for schools and institutions — helping them navigate the education landscape with clarity, confidence, and strategic direction.
              </p>
              <p className="text-[#2B303A]/75 leading-relaxed mb-8">
                Whether you are designing a new programme, strengthening your teaching workforce, or rethinking your institutional strategy, we partner with you to deliver tailored solutions that improve outcomes and drive sustainable growth.
              </p>
              <a href="/contact" className="inline-flex items-center gap-2 text-[#C5A059] font-semibold text-sm hover:gap-3 transition-all duration-200" style={{ fontFamily: "var(--font-mono)" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            {services.map((service, i) => (
              <AnimatedSection key={service.title} animation="slide-right" delay={i * 80}>
                <div className="group p-6 bg-white border border-[#EEF2F6] hover:border-[#C5A059]/30 hover:shadow-lg transition-all duration-200 rounded-sm h-full">
                  ...
                  <h3 className="font-display text-[#0D2346] font-bold text-base mb-2" style={{ fontFamily: "var(--font-display)" }}>{service.title}</h3>
                  <p className="text-[#2B303A]/70 text-sm leading-relaxed">{service.description}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
